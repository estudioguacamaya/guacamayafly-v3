...
const Home: React.FC = () => {
  ...
  useEffect(() => {
    // --- Lógica del script original migrada a useEffect ---
    
    // Global functions required for inline HTML calls (generated by JS)
    const win = window as any;

    win.scrollCarousel = function(carouselId: string, direction: number) {
    ...
    win.setupCarouselHopAnimation = function(carouselId: string) {
        const carousel = document.getElementById(carouselId);
        if (!carousel) return;
    ...
    win.startAutoplay = function(carouselId: string, interval = 4000) {
        const carousel = document.getElementById(carouselId) as any;
    ...
    win.handleCardBooking = function(type: any, name: any, details: any) {
    ...
    let onPopupConfirmCallback: (() => void) | null = null;

    function showDevelopmentPopup(callback?: () => void) {
        if (callback) onPopupConfirmCallback = callback;
        if (devPopupOverlay) {
    ...
    const originIataInput = document.getElementById('origin-iata') as HTMLInputElement;
    const destinationIataInput = document.getElementById('destination-iata') as HTMLInputElement;

    // NEW AMADEUS API KEYS
    const AMADEUS_API_KEY = "ohXhjtQD8uEGD2qhirAByxn2loyDmQJq";
    const AMADEUS_API_SECRET = "BQOMOO3rJHk3qXOm";
    let amadeusToken: any = null;
    let tokenExpiry: any = null;
    let currentFlightData = null;
    let currentSearchMode = 'flights';

    let currentMonthDate = new Date();
    let selectedDeparture: Date | null = null;
    let selectedReturn: Date | null = null;
    ...
    function searchLocations(keyword: string, inputElement: HTMLInputElement, suggestionsContainer: HTMLElement) {
        if (keyword.length < 2) {
    ...
    function displaySuggestions(suggestions: any[], inputElement: HTMLInputElement, suggestionsContainer: HTMLElement) {
        suggestionsContainer.innerHTML = '';
            ...
            suggestions.forEach(suggestion => {
                ...
                item.addEventListener('click', () => {
                    ...
                    if (inputElement.id === 'origin-input') {
                        if (originIataInput) originIataInput.value = suggestion.iata;
                    } else {
                        if (destinationIataInput) destinationIataInput.value = suggestion.iata;
                    }
                    suggestionsContainer.innerHTML = '';
                    suggestionsContainer.classList.add('hidden');
                });
    ...
    if (originInput) originInput.addEventListener('input', () => searchLocations(originInput.value, originInput, document.getElementById('origin-suggestions')!));
    if (destinationInput) destinationInput.addEventListener('input', () => searchLocations(destinationInput.value, destinationInput, document.getElementById('destination-suggestions')!));
    document.addEventListener('click', (e) => {
        if (originInput && !originInput.parentElement?.contains(e.target as Node)) document.getElementById('origin-suggestions')?.classList.add('hidden');
        if (destinationInput && !destinationInput.parentElement?.contains(e.target as Node)) document.getElementById('destination-suggestions')?.classList.add('hidden');
    });


    async function searchFlights(origin: any, destination: any, departureDate: any, returnDate: any, adults: any, children: any) {
        if (!mainContent || !resultsContainer || !searchLoader || !cardsContainer || !resultsTitle) return;
    ...
    function renderFlightResults(flights: any[], dictionaries: any) {
        ...
        flights.forEach((flight, index) => {
            ...
            cardLink.dataset.basePrice = basePrice.toString();
            ...
            cardLink.querySelector('.select-flight-btn')!.addEventListener('click', (e) => {
                ...
                // Use window location for navigation in strict mode script
                window.location.href = '#/pagocheckout';
            });
    ...
    function updateAllFlightPrices() {
        document.querySelectorAll('#cards-container .flight-card-article').forEach(card => {
            ...
            const basePrice = parseFloat(c.dataset.basePrice || "0");
            if (!isNaN(basePrice)) {
                const newFinalPrice = calculateFinalPrice(basePrice);
                const priceEl = c.querySelector('.flight-price');
                if (priceEl) priceEl.textContent = `${newFinalPrice.toFixed(2)} €`;
            }
        });
    }
    ...
    function addPassengerForm() {
        ...
        const foreignerCheckbox = passengerDiv.querySelector('input[name="is-foreigner"]') as HTMLInputElement;
        const cedulaField = passengerDiv.querySelector('div[name="cedula-field"]') as HTMLElement;
        const passportField = passengerDiv.querySelector('div[name="passport-field"]') as HTMLElement;
        const cedulaInput = cedulaField.querySelector('input') as HTMLInputElement;
        const passportInput = passportField.querySelector('input') as HTMLInputElement;
    ...
    function updateDateDisplay() {
        if (dateRangeDisplay) {
            if (selectedDeparture && selectedReturn) {
                ...
                dateRangeDisplay.textContent = `${selectedDeparture.toLocaleDateString('es-ES', options)} - ${selectedReturn.toLocaleDateString('es-ES', options)}`;
                dateRangeDisplay.classList.add('text-black', 'font-semibold');
                ...
                if(calendarCheckinDisplay) calendarCheckinDisplay.textContent = selectedDeparture.toLocaleDateString('es-ES', {weekday: 'short', day: 'numeric', month: 'short'});
                if(calendarCheckoutDisplay) calendarCheckoutDisplay.textContent = selectedReturn.toLocaleDateString('es-ES', {weekday: 'short', day: 'numeric', month: 'short'});

            } else {
                dateRangeDisplay.textContent = "Fechas";
                dateRangeDisplay.classList.remove('text-black', 'font-semibold');
                dateRangeDisplay.classList.add('text-gray-500');
                 if(calendarCheckinDisplay) calendarCheckinDisplay.textContent = '--';
                if(calendarCheckoutDisplay) calendarCheckoutDisplay.textContent = '--';
            }
        }
    }

    function updateSearchFormUI(mode: string) {
        if(mode === 'flights') {
            document.getElementById('origin-container')?.classList.remove('hidden');
            document.getElementById('destination-container')?.classList.remove('hidden');
            if(originInput) originInput.placeholder = "Origen";
            if(destinationInput) destinationInput.placeholder = "Destino";
            const originIcon = document.getElementById('origin-icon');
            const destIcon = document.getElementById('destination-icon');
            if(originIcon) originIcon.className = "fas fa-plane-departure";
            if(destIcon) destIcon.className = "fas fa-plane-arrival";
        } else if (mode === 'hotels') {
            document.getElementById('origin-container')?.classList.add('hidden');
            document.getElementById('destination-container')?.classList.remove('hidden');
            if(destinationInput) destinationInput.placeholder = "Destino, hotel o lugar de interés";
            const destIcon = document.getElementById('destination-icon');
            if(destIcon) destIcon.className = "fas fa-map-marker-alt";
        } else {
             document.getElementById('origin-container')?.classList.remove('hidden');
             document.getElementById('destination-container')?.classList.remove('hidden');
        }
    ...
    function renderCalendar() {
        ...
        // Simple 1 or 2 month calendar rendering logic
        ...
        const firstDayIndex = new Date(date.getFullYear(), date.getMonth(), 1).getDay(); // 0 Sun, 1 Mon...
        // Convert to Mon start (0 Mon ... 6 Sun)
        const startDay = (firstDayIndex + 6) % 7;

        const monthDiv = document.createElement('div');
        monthDiv.className = 'calendar-month w-full';
        
        let html = `
            <div class="calendar-grid grid grid-cols-7 gap-1 text-center text-sm mb-2">
                <div class="font-bold text-gray-500">L</div>
                <div class="font-bold text-gray-500">M</div>
                <div class="font-bold text-gray-500">X</div>
                <div class="font-bold text-gray-500">J</div>
                <div class="font-bold text-gray-500">V</div>
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            let classes = 'p-2 rounded-full cursor-pointer hover:bg-gray-100';
            if (isPast) classes = 'p-2 text-gray-300 cursor-not-allowed';
            
             // Selection logic
            if (!isPast) {
                if (selectedDeparture && currentDayDate.getTime() === selectedDeparture.getTime()) classes = 'p-2 rounded-full bg-blue-600 text-white';
                if (selectedReturn && currentDayDate.getTime() === selectedReturn.getTime()) classes = 'p-2 rounded-full bg-blue-600 text-white';
                if (selectedDeparture && selectedReturn && currentDayDate > selectedDeparture && currentDayDate < selectedReturn) classes = 'p-2 bg-blue-100';
            }

            html += `<div class="${classes} calendar-day" data-date="${currentDayDate.toISOString()}">${day}</div>`;
        }
        ...
        monthDiv.querySelectorAll('.calendar-day').forEach(el => {
            if(!el.classList.contains('cursor-not-allowed')) {
                el.addEventListener('click', (e) => {
                    const target = e.target as HTMLElement;
                    const d = new Date(target.dataset.date!);
                    if (!selectedDeparture || (selectedDeparture && selectedReturn)) {
                        selectedDeparture = d;
                        selectedReturn = null;
                    } else if (d < selectedDeparture) {
                        selectedDeparture = d;
                    } else {
                        selectedReturn = d;
    ...
    function updateTravelersDisplay() {
        if (travelersDisplay) travelersDisplay.textContent = `${adultsCount + childrenCount} viajero${(adultsCount + childrenCount) !== 1 ? 's' : ''}`;
        if (adultsCountSpan) adultsCountSpan.textContent = adultsCount.toString();
        if (childrenCountSpan) childrenCountSpan.textContent = childrenCount.toString();
        if (calendarGuestsDisplay) calendarGuestsDisplay.textContent = `${adultsCount + childrenCount}`;
        
        const decAdults = document.querySelector('[data-type="adults"][data-action="decrement"]') as HTMLButtonElement;
        if (decAdults) decAdults.disabled = adultsCount <= 1;
        const decChildren = document.querySelector('[data-type="children"][data-action="decrement"]') as HTMLButtonElement;
        if (decChildren) decChildren.disabled = childrenCount <= 0;
    }

    function updateBaggageDisplay() {
        if (baggageDisplay) {
            const totalBags = baggage10kgCount + baggage23kgCount;
            baggageDisplay.textContent = totalBags > 0 ? `${totalBags} maleta${totalBags !== 1 ? 's' : ''}` : 'Equipaje';
        }
        if (baggage10kgCountSpan) baggage10kgCountSpan.textContent = baggage10kgCount.toString();
        if (baggage23kgCountSpan) baggage23kgCountSpan.textContent = baggage23kgCount.toString();

        const dec10 = document.querySelector('[data-type="baggage-10kg"][data-action="decrement"]') as HTMLButtonElement;
        if (dec10) dec10.disabled = baggage10kgCount <= 0;
        const dec23 = document.querySelector('[data-type="baggage-23kg"][data-action="decrement"]') as HTMLButtonElement;
        if (dec23) dec23.disabled = baggage23kgCount <= 0;
    }

    function handleCounterClick(e: Event) {
        const target = e.target as HTMLElement;
        const btn = target.closest('button');
        if (!btn || !btn.classList.contains('counter-btn')) return;
    ...
    win.selectPopularFlight = (origin: any, destination: any, departureDate: any, returnDate: any) => {
    ...
    function populateCarousels() {
        ...
        const baseFlights = [ { airline: 'Iberia', stops: 'Sin escalas', from: 'Madrid (MAD)', to: 'Caracas (CCS)', fromIata: 'MAD', toIata: 'CCS', time1: '11:50 - 15:35', time2: '17:50 - 08:25', price: '950 €' }, { airline: 'Air Europa', stops: 'Sin escalas', from: 'Madrid (MAD)', to: 'Caracas (CCS)', fromIata: 'MAD', toIata: 'CCS', time1: '15:10 - 18:55', time2: '20:55 - 11:30', price: '980 €' }, { airline: 'Copa Airlines', stops: '1 escala', from: 'Santiago (SCL)', to: 'Caracas (CCS)', fromIata: 'SCL', toIata: 'CCS', time1: '01:30 - 11:00', time2: '15:00 - 00:45', price: '820 €' }, { airline: 'LATAM', stops: '1 escala', from: 'Bogotá (BOG)', to: 'Caracas (CCS)', fromIata: 'BOG', toIata: 'CCS', time1: '07:00 - 10:30', time2: '18:00 - 21:30', price: '450 €' }, { airline: 'Avianca', stops: '1 escala', from: 'Buenos Aires (EZE)', to: 'Caracas (CCS)', fromIata: 'EZE', toIata: 'CCS', time1: '06:00 - 15:00', time2: '17:00 - 02:00', price: '1100 €' }, { airline: 'Turkish Airlines', stops: '1 escala', from: 'Estambul (IST)', to: 'Caracas (CCS)', fromIata: 'IST', toIata: 'CCS', time1: '02:20 - 14:00', time2: '10:00 - 07:00', price: '1300 €' }, { airline: 'Plus Ultra', stops: 'Sin escalas', from: 'Madrid (MAD)', to: 'Caracas (CCS)', fromIata: 'MAD', toIata: 'CCS', time1: '13:00 - 16:45', time2: '18:45 - 09:15', price: '890 €' }, { airline: 'Wingo', stops: '1 escala', from: 'Medellín (MDE)', to: 'Caracas (CCS)', fromIata: 'MDE', toIata: 'CCS', time1: '05:30 - 09:00', time2: '16:00 - 19:30', price: '380 €' }, { airline: 'Sky High', stops: '1 escala', from: 'Miami (MIA)', to: 'Caracas (CCS)', fromIata: 'MIA', toIata: 'CCS', time1: '08:00 - 13:00', time2: '15:00 - 20:00', price: '650 €' }, { airline: 'Caribbean Airlines', stops: '1 escala', from: 'Puerto España (POS)', to: 'Caracas (CCS)', fromIata: 'POS', toIata: 'CCS', time1: '09:00 - 10:30', time2: '17:00 - 18:30', price: '320 €' }, { airline: 'Aerolíneas Argentinas', stops: '1 escala', from: 'Buenos Aires (AEP)', to: 'Caracas (CCS)', fromIata: 'AEP', toIata: 'CCS', time1: '09:00 - 17:00', time2: '19:00 - 03:00', price: '1150 €' }, { airline: 'Conviasa', stops: 'Sin escalas', from: 'La Habana (HAV)', to: 'Caracas (CCS)', fromIata: 'HAV', toIata: 'CCS', time1: '10:00 - 13:00', time2: '15:00 - 18:00', price: '550 €' }, { airline: 'Estelar', stops: 'Sin escalas', from: 'Santo Domingo (SDQ)', to: 'Caracas (CCS)', fromIata: 'SDQ', toIata: 'CCS', time1: '11:00 - 12:30', time2: '14:00 - 15:30', price: '410 €' }, { airline: 'Air France', stops: '1 escala', from: 'París (CDG)', to: 'Caracas (CCS)', fromIata: 'CDG', toIata: 'CCS', time1: '10:40 - 15:10', time2: '21:00 - 14:00', price: '1250 €' }, { airline: 'LATAM', stops: '1 escala', from: 'Lima (LIM)', to: 'Caracas (CCS)', fromIata: 'LIM', toIata: 'CCS', time1: '08:30 - 13:30', time2: '16:00 - 21:00', price: '600 €' } ];
            ...
            const basePackages = [ { name: 'Hostal Madrid Atocha', stars: '3', rating: '9.2/10 Increíble (752)', flight: 'Madrid (MAD)', save: '79 €', priceNew: '392', priceOld: '471', dates: 'De lun., 23 dic al Jue., 25 dic (3 noches)', updated: 'hace 45 horas', img: 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/386086064.jpg?k=865a09c08f297473a10052a8471861d18b68aeecaf7624675be2829889e88c45&o=' }, { name: 'Canal Wow Suites Amsterdam', stars: '4', rating: '9.2/10 Increíble (497)', flight: 'Amsterdam (AMS)', save: '89 €', priceNew: '686', priceOld: '722', dates: 'De Mar., 23 dic al jue., 25 sep (3 noches)', updated: 'hace 24 horas', img: 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/156865451.jpg?k=df167a737d2b9c2483bfa3462e2ec3f07ec26006794a0947205369f7361d5947&o=' }, { name: 'Ibis Budget Madrid Calle Alcalá', stars: '3', rating: '8.8/10 Excelente (46)', flight: 'Caraca Madrid (MAD)', save: '75 €', priceNew: '378', priceOld: '453', dates: 'De lun., 10 dic al jue., 13 dic (3 noches)', updated: 'hace 45 horas', img: 'https://images.getaroom-cdn.com/image/upload/s--2b3JbF-F--/c_limit,e_improve,fl_lossy.immutable_cache,h_940,q_auto:good,w_940/v1755855844/13c893a5cd0bfacac735eea4b9417616680b45d5?_a=BACAEuDL&atc=4e8c2e94' }, { name: 'Hotel Gracery Naha', stars: '4', rating: '9/10 Increíble (1.000)', flight: 'Tokio (HND) - Naha (OKA)', save: '194 €', priceNew: '500', priceOld: '694', dates: 'De mar., 17 dic al dom., 18 dic (1 noche)', updated: 'hace 47 horas', img: 'https://storage.googleapis.com/a1aa/image/ee5c710a-dee9-4dce-b960-ac814d36ff0d.jpg' }, { name: 'Paquete Roma Imperial', stars: '4', rating: '9.0/10 Excelente (850)', flight: 'Madrid (MAD) - Roma (FCO)', save: '120 €', priceNew: '480', priceOld: '600', dates: 'De vie., 12 dic al lun., 15 dic (3 noches)', updated: 'hace 2 horas', img: 'https://dynamic-media-cdn.tripadvisor.com/media/photo-o/08/26/a2/c5/mount-monserrate.jpg?w=1000&h=-1&s=1' }, { name: 'Escapada a Lisboa', stars: '3', rating: '8.5/10 Muy Bueno (600)', flight: 'Barcelona (BCN) - Lisboa (LIS)', save: '50 €', priceNew: '250', priceOld: '300', dates: 'De jue., 01 dic al dom., 04 dic (3 noches)', updated: 'hace 8 horas', img: 'https://dynamic-media-cdn.tripadvisor.com/media/photo-o/17/21/8b/a4/photo0jpg.jpg?w=1000&h=-1&s=1' }, { name: 'Aventura en Los Roques', stars: '4', rating: '9.8/10 Paradisíaco (1200)', flight: 'Caracas (CCS) - Los Roques (LRV)', save: '150 €', priceNew: '800', priceOld: '950', dates: 'De lun., 01 dic al vie., 05 dic (4 noches)', updated: 'hace 3 hras', img: 'https://dynamic-media-cdn.tripadvisor.com/media/photo-o/24/e0/5f/90/fachada.jpg?w=1000&h=-1&s=1' }, { name: 'Descubre Canaima', stars: '5', rating: '9.9/10 Único (980)', flight: 'Caracas (CCS) - Canaima (CAJ)', save: '200 €', priceNew: '1200', priceOld: '1400', dates: 'De sáb., 10 dic al mié., 15 dic (5 noches)', updated: 'hace 1 hora', img: 'https://dynamic-media-cdn.tripadvisor.com/media/photo-o/21/a2/be/db/giardino.jpg?w=1000&h=-1&s=1' }, { name: 'Magia Andina en Mérida', stars: '3', rating: '9.1/10 Encantador (750)', flight: 'Valencia (VLN) - El Vigía (VIG)', save: '80 €', priceNew: '400', priceOld: '480', dates: 'De vie., 20 dic al mar., 24 dic (4 noches)', updated: 'hace 12 horas', img: 'https://dynamic-media-cdn.tripadvisor.com/media/photo-o/2c/21/54/38/standard-parque-de-la.jpg?w=1000&h=-1&s=1' }, { name: 'Sol y Playa en Margarita', stars: '4', rating: '8.7/10 Divertido (1500)', flight: 'Maracaibo (MAR) - Porlamar (PMV)', save: '100 €', priceNew: '550', priceOld: '650', dates: 'De mié., 15 jul al lun., 20 jul (5 noches)', updated: 'hace 24 horas', img: 'https://dynamic-media-cdn.tripadvisor.com/media/photo-o/18/1a/6f/bb/gran-salon.jpg?w=1000&h=-1&s=1' }, { name: 'Paquete Tango en Buenos Aires', stars: '4', rating: '9.3/10 Apasionante (1100)', flight: 'Santiago (SCL) - Buenos Aires (EZE)', save: '130 €', priceNew: '650', priceOld: '780', dates: 'De jue., 05 nov al lun., 09 nov (4 noches)', updated: 'hace 6 horas', img: 'https://dynamic-media-cdn.tripadvisor.com/media/photo-o/05/f0/77/7c/hotel-estelar-calle-100.jpg?w=900&h=-1&s=1' }, { name: 'Café y Cultura en Bogotá', stars: '3', rating: '8.9/10 Interesante (880)', flight: 'Caracas (CCS) - Bogotá (BOG)', save: '70 €', priceNew: '380', priceOld: '450', dates: 'De mar., 01 dic al sáb., 05 dic (4 noches)', updated: 'hace 18 horas', img: 'https://dynamic-media-cdn.tripadvisor.com/media/photo-o/03/e2/da/9e/north-michigan-avenue.jpg?w=1000&h=-1&s=1' }, { name: 'Historia y Sabor en Lima', stars: '4', rating: '9.0/10 Delicioso (920)', flight: 'Bogotá (BOG) - Lima (LIM)', save: '90 €', priceNew: '520', priceOld: '610', dates: 'De lun., 18 ene al vie., 22 ene (4 noches)', updated: 'hace 1 día', img: 'https://dynamic-media-cdn.tripadvisor.com/media/photo-o/1b/c4/d1/f4/the-palm-house-at-65.jpg?w=1000&h=-1&s=1' }, { name: 'Vibras de Estambul', stars: '4', rating: '9.4/10 Mágico (1300)', flight: 'Madrid (MAD) - Estambul (IST)', save: '110 €', priceNew: '700', priceOld: '810', dates: 'De dom., 10 may al jue., 14 may (4 noches)', updated: 'hace 9 horas', img: 'https://dynamic-media-cdn.tripadvisor.com/media/photo-o/1a/c8/77/4e/bars-attractions-art.jpg?w=1000&h=-1&s=1' }, { name: 'Descanso en Punta Cana', stars: '5', rating: '9.5/10 Relax Total (2500)', flight: 'Caracas (CCS) - Punta Cana (PUJ)', save: '180 €', priceNew: '950', priceOld: '1130', dates: 'De sáb., 25 abr al jue., 30 abr (5 noches)', updated: 'hace 5 horas', img: 'https://dynamic-media-cdn.tripadvisor.com/media/photo-o/1a/41/2e/13/photo0jpg.jpg?w=1000&h=-1&s=1' } ];
            ...
            if (flightCarousel) {
                ...
                win.setupCarouselHopAnimation('flight-carousel');
            }
            ...
            if (accommodationCarousel) {
                ...
                win.setupCarouselHopAnimation('accommodation-carousel');
            }
            if (popularAccommodationsCarousel) {
                ...
                win.setupCarouselHopAnimation('popular-accommodations-carousel');
            }
            if (offersCarousel) {
                offersCarousel.innerHTML = baseOffers.map(o => `
                    <a href="#/productdetails.html" class="flex-shrink-0 w-64 bg-white text-gray-900 rounded-lg p-3 interactive-card flex flex-col cursor-pointer"
                       ...
                       data-property-price="${parseFloat(o.priceNew ?? "0")}">
                ...
                win.setupCarouselHopAnimation('offers-carousel');
            }
            if (vacationPackagesCarousel) {
                ...
                win.setupCarouselHopAnimation('vacation-packages-carousel');
            }
            if (specialAccommodationsCarousel) {
                ...
                win.setupCarouselHopAnimation('special-accommodations-carousel');
            }
            if (experiencesCarousel) {
                ...
                win.setupCarouselHopAnimation('experiences-carousel');
            }
            if (hotelDealsCarousel) {
                ...
                win.setupCarouselHopAnimation('hotel-deals-carousel');
            }

            // Start autoplay for all carousels
            win.startAutoplay('flight-carousel', 5000);
            win.startAutoplay('accommodation-carousel', 4500);
            win.startAutoplay('popular-accommodations-carousel', 5000);
            win.startAutoplay('offers-carousel', 4000);
            win.startAutoplay('vacation-packages-carousel', 5500);
            win.startAutoplay('special-accommodations-carousel', 4800);
            win.startAutoplay('hotel-deals-carousel', 4200);
            win.startAutoplay('experiences-carousel', 4700);
        }
    ...
    function handleCounterClick(e: Event) {
        ...
        if (clearSearchBtn) {
            clearSearchBtn.addEventListener('click', () => {
                ...
                if (originIataInput) originIataInput.value = '';
                if (destinationIataInput) destinationIataInput.value = '';
            });
        }
        ...
        document.querySelectorAll('.search-nav-button').forEach(button => {
            button.addEventListener('click', (e) => {
                ...
                if (newMode === 'all' || newMode === 'flights' || newMode === 'hotels') {
                    ...
                    document.querySelectorAll('.search-nav-button').forEach(b => b.classList.remove('active'));
        ...
        document.addEventListener('click', function(event) { if (travelersPickerTrigger && !travelersPickerTrigger.contains(event.target as Node) && travelersModal && !travelersModal.contains(event.target as Node)) { travelersModal.classList.add('hidden'); travelersModal.classList.remove('visible'); } if (baggagePickerTrigger && !baggagePickerTrigger.contains(event.target as Node) && baggageModal && !baggageModal.contains(event.target as Node)) { baggageModal.classList.add('hidden'); baggageModal.classList.remove('visible'); } });

        if (adultsCountSpan) { updateTravelersDisplay(); (document.querySelector('[data-type="adults"][data-action="decrement"]') as HTMLButtonElement).disabled = true; (document.querySelector('[data-type="children"][data-action="decrement"]') as HTMLButtonElement).disabled = true; }
        if (baggageDisplay) { updateBaggageDisplay(); (document.querySelector('[data-type="baggage-10kg"][data-action="decrement"]') as HTMLButtonElement).disabled = true; (document.querySelector('[data-type="baggage-23kg"][data-action="decrement"]') as HTMLButtonElement).disabled = true; }
        ...
        function goToSlide(index: number) { if (!sliderContainer) return; sliderContainer.style.transform = `translateX(-${index * 100}%)`; document.querySelectorAll('.slider-dot').forEach((dot, i) => dot.classList.toggle('active', i === index)); currentSlide = index; }
        ...
        allPropertyLinks.forEach(link => {
            link.addEventListener('click', (event) => {
                ...
                if (el.dataset.propertyId && el.dataset.propertyName && el.dataset.propertyImage && el.dataset.propertyPrice) {
                    const propertyId = el.dataset.propertyId;
                    const propertyName = el.dataset.propertyName;
                    const propertyImage = el.dataset.propertyImage;
                    const propertyPrice = el.dataset.propertyPrice;
  ...
  return (
    <div className="bg-white text-gray-900">
      <div id="development-popup-overlay" className="modal-overlay hidden" style={{zIndex: 12000}}>
          <div className="modal-container bg-white p-8 rounded-lg shadow-xl max-w-sm w-full text-center">
              ...
              <button id="development-popup-close" className="bg-blue-600 text-white font-semibold px-8 py-2 rounded-lg hover:bg-blue-700 transition-colors">
                  Entendido
              </button>
          </div>
      </div>
      ...
      <div id="loader" className="loader-overlay"><i className="fas fa-plane loader-plane"></i></div>
      
      <div id="calendar-modal" className="modal-overlay hidden calendar-modal">
          <div className="modal-container">
              <div className="calendar-modal-header">
                   <h3 className="font-semibold text-lg">Selecciona fechas</h3>
                   <button id="close-calendar-modal-x" className="p-2 rounded-full hover:bg-gray-200 text-2xl leading-none">&times;</button>
              </div>
              <div className="calendar-modal-top-info">
                  <div className="info-box">
                      <span className="label">Check-in</span>
                      <span id="calendar-checkin-display" className="value">--</span>
                  </div>
                  <div className="info-box">
                      <span className="label">Check-out</span>
                      <span id="calendar-checkout-display" className="value">--</span>
                  </div>
                  <div className="info-box">
                      <span className="label">Huéspedes</span>
                      <span id="calendar-guests-display" className="value">--</span>
                  </div>
              </div>
              <div className="calendar-modal-body">
                  ...
                  <div className="calendar-header">
                      <button id="prev-month" className="p-2 rounded-full hover:bg-gray-200"><i className="fas fa-chevron-left"></i></button>
                      <h4 id="calendar-month-year" className="font-semibold text-lg"></h4>
                      <button id="next-month" className="p-2 rounded-full hover:bg-gray-200"><i className="fas fa-chevron-right"></i></button>
                  </div>
                  <div className="calendar-months" id="calendar-months-container"></div>
                  <div className="price-legend">
                      <div><div className="color-box" style={{backgroundColor: '#d1fae5'}}></div><span>Más barato</span></div>
                      <div><div className="color-box" style={{backgroundColor: '#fee2e2'}}></div><span>Más caro</span></div>
                  </div>
                  <p className="text-center text-xs text-gray-500 mt-2">Según los precios promedio</p>
              ...
              <div className="calendar-modal-footer">
                  <button id="clear-dates-btn" className="bg-gray-200 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-300 font-semibold">Limpiar</button>
                  <button id="close-calendar-btn" className="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 font-semibold">Solicitar</button>
              </div>
          </div>
      </div>
      
      <div id="booking-modal" className="modal-overlay hidden">
          <div className="modal-container max-w-4xl max-h-[90vh] overflow-y-auto">
               <div className="flex justify-between items-center mb-6">
                  <h3 className="font-semibold text-xl">Confirmar Reserva de Vuelo</h3>
                  <button id="close-booking-modal" className="p-2 rounded-full hover:bg-gray-200 text-2xl leading-none">&times;</button>
              </div>
              <div className="flex flex-col md:flex-row gap-8">
                  <div className="md:w-3/5">
                      <form id="booking-form">
                          <div className="mb-6">
                              <h4 className="font-bold text-gray-800 mb-3 text-lg">Datos del Titular de la Reserva</h4>
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                  <div>
                                      <label htmlFor="contact-email" className="block text-sm font-medium text-gray-700">Correo Electrónico</label>
                                      <input type="email" id="contact-email" className="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-guacamayafly-orange focus:border-guacamayafly-orange sm:text-sm" required />
                                  </div>
                                  <div>
                                      <label htmlFor="contact-phone" className="block text-sm font-medium text-gray-700">Teléfono</label>
                                      <input type="tel" id="contact-phone" className="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-guacamayafly-orange focus:border-guacamayafly-orange sm:text-sm" required />
                                  </div>
                                  <div className="sm:col-span-2">
                                       <label htmlFor="contact-source" className="block text-sm font-medium text-gray-700">¿Dónde nos conoció?</label>
                                       <select id="contact-source" className="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-guacamayafly-orange focus:border-guacamayafly-orange sm:text-sm" required>
                          ...
                          <div>
                              <h4 className="font-bold text-gray-800 mb-3 text-lg">Pasajeros</h4>
                              <div id="passengers-container" className="space-y-6">
                                  </div>
                              <button type="button" id="add-passenger-btn" className="w-full text-sm text-blue-600 hover:text-blue-800 font-semibold py-2 mt-4">+ Añadir otro viajero</button>
                          </div>
                      </form>
                  </div>
                  <div className="md:w-2/5">
                      <h4 className="font-bold text-gray-800 mb-3 text-lg">Resumen y Pago</h4>
                      <div id="booking-flight-summary" className="mb-6 p-4 bg-gray-100 rounded-lg text-sm"></div>
                      <div>
                           <label className="block text-sm font-medium text-gray-700 mb-2">Método de Pago</label>
                           <div className="space-y-3" id="flight-payment-options-container">
                              <div className="payment-option" data-payment="whatsapp">
                                  <div className="flex items-center justify-between">
                                      <span className="font-semibold text-sm">Reservar por WhatsApp</span>
                                      <i className="fab fa-whatsapp text-xl text-green-500"></i>
                                  </div>
                              </div>
                              <div className="payment-option" data-payment="card">
                                  <div className="flex items-center justify-between">
                                      <span className="font-semibold text-sm">Tarjeta de Crédito/Débito</span>
                                      <div className="flex items-center gap-2">
                                          <img src="https://cryptoindex.com/pic/icons/visa-mastercard.svg" alt="Visa y Mastercard" className="payment-logo" />
                                      </div>
                                  </div>
                              </div>
                              <div className="payment-option" data-payment="crypto">
                                   <div className="flex items-center justify-between">
                                      <span className="font-semibold text-sm">Pagar con Criptomonedas</span>
                                      <img src="https://www.bybit.com/bycsi-root/assets/image/coins/light/btc.svg?quality=63&format=avif&resize=width/50.4" alt="Bitcoin" className="payment-logo" />
                                  </div>
                              </div>
                           </div>
                      </div>
      ...
      <div id="hotel-card-booking-modal" className="modal-overlay hidden calendar-modal">
          <div className="modal-container">
              <div className="calendar-modal-header">
                  <h3 id="hotel-modal-title" className="font-semibold text-lg">Reservar Hotel</h3>
                  <button id="close-hotel-modal-btn" className="p-2 rounded-full hover:bg-gray-200 text-2xl leading-none">&times;</button>
              </div>
              <div id="hotel-calendar-top-info" className="calendar-modal-top-info">
                  </div>
              <div className="calendar-modal-body">
                  <div id="hotel-modal-calendar-container">
                      </div>
              </div>
              <div id="hotel-modal-form-container" className="p-4 border-t overflow-y-auto" style={{maxHeight: '35vh'}}>
                   <form id="hotel-modal-form">
                      <input type="hidden" id="hotel-modal-name" />
                      <h4 className="font-bold text-gray-700 mb-2">Datos del Titular</h4>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                          <div>
                              <label htmlFor="hotel-modal-firstname" className="block text-sm font-medium text-gray-700">Nombre</label>
                              <input type="text" id="hotel-modal-firstname" className="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-guacamayafly-orange focus:border-guacamayafly-orange sm:text-sm" required />
                          </div>
                          <div>
                              <label htmlFor="hotel-modal-lastname" className="block text-sm font-medium text-gray-700">Apellido</label>
                              <input type="text" id="hotel-modal-lastname" className="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-guacamayafly-orange focus:border-guacamayafly-orange sm:text-sm" required />
                          </div>
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                          <div>
                              <label htmlFor="hotel-modal-email" className="block text-sm font-medium text-gray-700">Correo Electrónico</label>
                              <input type="email" id="hotel-modal-email" className="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-guacamayafly-orange focus:border-guacamayafly-orange sm:text-sm" required />
                          </div>
                          <div>
                              <label htmlFor="hotel-modal-phone" className="block text-sm font-medium text-gray-700">Teléfono</label>
                              <input type="tel" id="hotel-modal-phone" className="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-guacamayafly-orange focus:border-guacamayafly-orange sm:text-sm" required />
                          </div>
                      </div>
                      <div className="mt-6">
                           <label className="block text-sm font-medium text-gray-700 mb-2">Método de Pago</label>
                           <div className="space-y-3" id="payment-options-container">
                              <div className="payment-option" data-payment="whatsapp">
                                  <div className="flex items-center justify-between">
                                      <span className="font-semibold text-sm">Reservar por WhatsApp</span>
                                      <i className="fab fa-whatsapp text-xl text-green-500"></i>
      ...
      <div id="payment-info-popup" className="payment-info-popup"></div>
      ...
      <header className="flex items-center justify-between px-6 py-4 max-w-[1200px] mx-auto">
          ...
          <nav className="hidden sm:flex space-x-6 text-sm font-normal text-black">
              ...
              <Link className="hover:underline" to="/pagocheckout.html">Pagos</Link> 
          </nav>
      ...
      <main id="main-container" className="max-w-[1200px] mx-auto px-6">
          <div id="main-content">
               <section className="text-center my-12">
                  <h1 id="search-title" className="font-extrabold text-2xl sm:text-3xl md:text-4xl mt-6 mb-6">Encuentra el mejor vuelo</h1>
                  <nav id="search-nav" className="flex flex-wrap justify-center gap-4 text-xs sm:text-sm font-normal text-gray-700 mb-4">
                       <button data-mode="all" className="search-nav-button flex items-center gap-1 border border-gray-700 rounded px-2 py-1 font-semibold"> <i className="fas fa-home"></i> Buscar todo </button>
                       <button data-mode="hotels" className="search-nav-button flex items-center gap-1"> <i className="fas fa-bed"></i> Hoteles </button>
                       <button data-mode="flights" className="search-nav-button flex items-center gap-1 border border-black rounded px-2 py-1 font-semibold active"> <i className="fas fa-plane"></i> Vuelos </button>
                       <button data-mode="tours" className="search-nav-button flex items-center gap-1"> <i className="fas fa-camera"></i> Cosas que hacer </button>
                       <button data-mode="restaurants" className="search-nav-button flex items-center gap-1"> <i className="fas fa-utensils"></i> Restaurantes </button>
                       <button data-mode="rentals" className="search-nav-button flex items-center gap-1"> <i className="fas fa-key"></i> Alquileres vacacionales </button>
                  </nav>
                  <form id="search-form" aria-label="Buscar vuelos" className="search-form-container mb-12" role="search" noValidate>
                      <div id="origin-container" className="search-input-group">
                          <i id="origin-icon" className="fas fa-plane-departure"></i>
                          <input id="origin-input" placeholder="Origen" type="text" required autoComplete="off"/>
                          <div id="origin-suggestions" className="autocomplete-suggestions hidden"></div>
                      </div>
                      <div id="destination-container" className="search-input-group">
                          ...
                          <input id="destination-input" placeholder="Destino" type="text" required autoComplete="off"/>
                           <div id="destination-suggestions" className="autocomplete-suggestions hidden"></div>
                      </div>
                      <div id="date-picker-trigger" className="search-input-group cursor-pointer">
                          <i className="fas fa-calendar-alt"></i>
                          <span id="date-range-display" className="text-gray-500">Fechas</span>
                      </div>
                      <div id="travelers-picker-trigger" className="search-input-group cursor-pointer">
                          <i className="fas fa-user"></i>
                          <span id="travelers-display">1 viajero</span>
                          <div id="travelers-modal" className="popover-modal hidden">
                              <div className="popover-row">
                                  <div>
                                      <p className="font-semibold">Adultos</p>
                                      <p className="text-xs text-gray-500">18 años o más</p>
                                  </div>
                                  <div className="flex items-center gap-4">
                                      <button type="button" className="counter-btn" data-type="adults" data-action="decrement">-</button>
                                      <span id="adults-count" className="font-semibold text-lg">1</span>
                                      <button type="button" className="counter-btn" data-type="adults" data-action="increment">+</button>
                                  </div>
                              </div>
                              <div className="popover-row">
                                   <div>
                                      <p className="font-semibold">Niños</p>
                                      <p className="text-xs text-gray-500">0 a 17 años</p>
                                  </div>
                                  <div className="flex items-center gap-4">
                                      <button type="button" className="counter-btn" data-type="children" data-action="decrement">-</button>
                                      <span id="children-count" className="font-semibold text-lg">0</span>
                                      <button type="button" className="counter-btn" data-type="children" data-action="increment">+</button>
                                  </div>
                              </div>
                              <button type="button" id="close-travelers-btn" className="w-full bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 font-semibold mt-2">Aceptar</button>
                          </div>
                      </div>
                       <div id="baggage-picker-trigger" className="search-input-group cursor-pointer">
                          <i className="fas fa-suitcase-rolling"></i>
                          <span id="baggage-display">Equipaje</span>
                          <div id="baggage-modal" className="popover-modal hidden">
                              <div className="popover-row">
                                  <div>
                                      <p className="font-semibold">Equipaje de mano (10kg)</p>
                                      <p className="text-xs text-gray-500">Costo: 45.00 € por pieza</p>
                                  </div>
                                  <div className="flex items-center gap-4">
                                      <button type="button" className="counter-btn" data-type="baggage-10kg" data-action="decrement">-</button>
                                      <span id="baggage-10kg-count" className="font-semibold text-lg">0</span>
                                      <button type="button" className="counter-btn" data-type="baggage-10kg" data-action="increment">+</button>
                                  </div>
                              </div>
                              <div className="popover-row">
                                   <div>
                                      <p className="font-semibold">Equipaje facturado (23kg)</p>
                                      <p className="text-xs text-gray-500">Costo: 85.00 € por pieza</p>
                                  </div>
                                  <div className="flex items-center gap-4">
                                      <button type="button" className="counter-btn" data-type="baggage-23kg" data-action="decrement">-</button>
                                      <span id="baggage-23kg-count" className="font-semibold text-lg">0</span>
                                      <button type="button" className="counter-btn" data-type="baggage-23kg" data-action="increment">+</button>
                                  </div>
                              </div>
                              <button type="button" id="close-baggage-btn" className="w-full bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 font-semibold mt-2">Aceptar</button>
                          </div>
                      </div>
                      ...
                      <button className="search-main-button" type="submit">
                          <i className="fas fa-search"></i>
                      </button>
                  </form>
              </section>
              
              <section id="hero-section" aria-label="Los mejores hoteles del mundo, elegidos por ti" className="relative rounded-lg overflow-hidden mb-12 max-w-[1200px] mx-auto section-hero-image">
                  <div id="hero-slider-container" className="h-full">
                      </div>
              ...
              <section aria-label="Recent searches" className="mb-12">
                  ...
                  <Link to="/productdetails.html" className="flex items-center bg-gray-100 rounded-lg p-3 max-w-xs text-xs text-gray-700 font-semibold cursor-pointer interactive-card">
              ...
              <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-12">
                  ...
                  <nav className="border-b border-gray-200 max-w-4xl mb-4">
                   ...
                   <div className="carousel-container max-w-4xl mx-auto">
                      <div id="flight-carousel" className="flex space-x-4 scrollbar-hide pb-2 carousel-content-wrapper">
                          </div>
                      <button aria-label="Desplazar vuelos a la izquierda" className="carousel-nav-button left" onClick={() => (window as any).scrollCarousel('flight-carousel', -1)}><i className="fas fa-chevron-left"></i></button>
                      <button aria-label="Desplazar vuelos a la derecha" className="carousel-nav-button right" onClick={() => (window as any).scrollCarousel('flight-carousel', 1)}><i className="fas fa-chevron-right"></i></button>
                  </div>
              </section>
              ...
              <section className="text-center mb-12">
                  ...
                  <div className="carousel-container max-w-[1200px] mx-auto">
                      <div id="accommodation-carousel" className="flex space-x-4 scrollbar-hide pb-2 carousel-content-wrapper">
                          </div>
                      <button aria-label="Desplazar alojamientos a la izquierda" onClick={() => (window as any).scrollCarousel('accommodation-carousel', -1)} className="carousel-nav-button left"><i className="fas fa-chevron-left"></i></button>
                      <button aria-label="Desplazar alojamientos a la derecha" onClick={() => (window as any).scrollCarousel('accommodation-carousel', 1)} className="carousel-nav-button right"><i className="fas fa-chevron-right"></i></button>
                  </div>
              </section>
              ...
              <section className="relative mt-8 rounded-2xl overflow-hidden mb-12">
                  ...
                  <div className="relative p-6 sm:p-8 lg:p-10 offers-text-overlay">
                      ...
                      <div className="carousel-container">
                          <div id="offers-carousel" className="flex space-x-4 scrollbar-hide pb-2 carousel-content-wrapper">
                             </div>
                          <button aria-label="Desplazar ofertas a la izquierda" onClick={() => (window as any).scrollCarousel('offers-carousel', -1)} className="carousel-nav-button left"><i className="fas fa-chevron-left"></i></button>
                          <button aria-label="Desplazar ofertas a la derecha" onClick={() => (window as any).scrollCarousel('offers-carousel', 1)} className="carousel-nav-button right"><i className="fas fa-chevron-right"></i></button>
                      </div>
                  </div>
              </section>
              ...
              <section className="max-w-[1200px] mx-auto my-12">
                 ...
                 <div className="carousel-container">
                      <div id="vacation-packages-carousel" className="flex space-x-4 scrollbar-hide pb-2 carousel-content-wrapper">
                          </div>
                      <button aria-label="Siguiente" onClick={() => (window as any).scrollCarousel('vacation-packages-carousel', 1)} className="carousel-nav-button right"><i className="fas fa-chevron-right fa-lg"></i></button>
                      <button aria-label="Anterior" onClick={() => (window as any).scrollCarousel('vacation-packages-carousel', -1)} className="carousel-nav-button left"><i className="fas fa-chevron-left fa-lg"></i></button>
                  </div>
      ...
      <section className="text-center mb-12">
                  ...
                  <nav className="flex space-x-4 border-b border-gray-200 mb-4 text-sm font-semibold text-blue-700 justify-center">
                      ...
                      <button className="pb-1 opacity-70 hover:opacity-100">Esquí</button>
                      <button className="pb-1 opacity-70 hover:opacity-100">Familia</button>
                      <button className="pb-1 opacity-70 hover:opacity-100">Bienestar</button>
                  </nav>
                  <div className="carousel-container max-w-[1200px] mx-auto">
                      <div id="popular-accommodations-carousel" className="flex space-x-4 scrollbar-hide pb-2 carousel-content-wrapper">
                          </div>
                      <button aria-label="Desplazar destinos a la izquierda" onClick={() => (window as any).scrollCarousel('popular-accommodations-carousel', -1)} className="carousel-nav-button left"><i className="fas fa-chevron-left"></i></button>
                      <button aria-label="Desplazar destinos a la derecha" onClick={() => (window as any).scrollCarousel('popular-accommodations-carousel', 1)} className="carousel-nav-button right"><i className="fas fa-chevron-right"></i></button>
                  </div>
              </section>
              <section className="my-12 text-center">
                  ...
                  <p className="text-xs sm:text-sm text-gray-600 mb-3 max-w-[700px] mx-auto">
                      Se muestran ofertas para estas fechas:
                      <span className="font-semibold">20 Dic - 22 Dic</span>
                  </p>
                  <div className="carousel-container">
                      <div id="special-accommodations-carousel" className="flex space-x-3 scrollbar-hide pb-2 carousel-content-wrapper">
                          </div>
                      <button aria-label="Desplazar alojamientos especiales a la izquierda" onClick={() => (window as any).scrollCarousel('special-accommodations-carousel', -1)} className="carousel-nav-button left"><i className="fas fa-chevron-left"></i></button>
                      <button aria-label="Desplazar alojamientos especiales a la derecha" onClick={() => (window as any).scrollCarousel('special-accommodations-carousel', 1)} className="carousel-nav-button right"><i className="fas fa-chevron-right"></i></button>
                  </div>
              </section>
              ...
              <section className="my-12 text-center">
                  ...
                  <div className="carousel-container max-w-[1200px] mx-auto">
                      <div id="hotel-deals-carousel" className="flex space-x-4 scrollbar-hide pb-2 carousel-content-wrapper">
                          </div>
                       <button aria-label="Siguiente" onClick={() => (window as any).scrollCarousel('hotel-deals-carousel', 1)} className="carousel-nav-button right"><i className="fas fa-chevron-right fa-lg"></i></button>
                      <button aria-label="Anterior" onClick={() => (window as any).scrollCarousel('hotel-deals-carousel', -1)} className="carousel-nav-button left"><i className="fas fa-chevron-left fa-lg"></i></button>
                  </div>
              </section>
              ...
              <section className="my-12 text-center">
                  ...
                  <div className="carousel-container max-w-[1200px] mx-auto">
                      <div id="experiences-carousel" className="flex space-x-4 scrollbar-hide pb-2 carousel-content-wrapper">
                          </div>
                      <button aria-label="Siguiente" onClick={() => (window as any).scrollCarousel('experiences-carousel', 1)} className="carousel-nav-button right"><i className="fas fa-chevron-right fa-lg"></i></button>
                      <button aria-label="Anterior" onClick={() => (window as any).scrollCarousel('experiences-carousel', -1)} className="carousel-nav-button left"><i className="fas fa-chevron-left fa-lg"></i></button>
                  </div>
              </section>
          ...
          <div id="results-container" className="hidden max-w-6xl mx-auto mt-8">
              <div className="flex justify-between items-center mb-4">
                  <h2 id="results-title" className="text-2xl font-bold"></h2>
                  <button id="clear-search-btn" className="bg-gray-200 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-300 font-semibold">Limpiar Búsqueda</button>
              </div>
              <div id="search-loader" className="flight-search-loader hidden"><i className="fas fa-spinner fa-spin"></i><p>Buscando las mejores ofertas...</p></div>
              <div id="cards-container" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6"></div>
          </div>
      
          <div id="crypto-checkout-section" className="hidden max-w-4xl mx-auto py-12">
               <div className="flex justify-between items-center mb-6">
                  <h2 className="text-3xl font-bold text-gray-800">Finalizar Pago</h2>
                  <button id="close-crypto-checkout-btn" className="text-gray-500 hover:text-gray-800">
                      <i className="fas fa-times fa-lg"></i>&nbsp;Volver
                  </button>
              </div>
              <div className="bg-gray-50 p-6 rounded-lg shadow-md">
                  <div className="flex flex-col md:flex-row gap-8">
                      <div className="md:w-2/3">
                          <h3 className="font-bold text-gray-700 mb-3 text-lg">Resumen de la Reserva</h3>
                          <div id="crypto-summary-details" className="text-sm space-y-2 text-gray-600">
                              </div>
                      </div>
                      <div className="md:w-1/2 flex flex-col items-center">
                          <p className="font-semibold text-gray-700 mb-2">Escanear para Pagar</p>
                          <div className="p-3 border rounded-lg bg-white shadow-sm">
                              <img src="https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=bc1qxy2kgdygjrsqtzq2n0yrf2493p83kkfjhx0wlh" alt="Código QR de pago" id="crypto-qr-code-img" className="w-40 h-40" />
                          </div>
                           <p className="text-xs text-gray-500 mt-2">Use su billetera para escanear</p>
                      </div>
                  </div>
                  <div className="mt-6 border-t pt-6">
                      <h3 className="font-bold text-gray-700 mb-3 text-lg">O Pagar a la Siguiente Dirección</h3>
                      <div id="crypto-wallets-list" className="space-y-3">
                          </div>
                  </div>
              </div>
          </div>
